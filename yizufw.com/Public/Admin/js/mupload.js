...
layui.define(['element', 'jquery', 'layer'] , function(exports){
    ...
    function add_file_item(container, key, name){
        ...
        tr.push('<i class="layui-item-going" style="font-size: 14px;display:none;color: #808080;">正在上传...</i>');
        tr.push('<i class="layui-icon layui-item-success" style="font-size: 20px;color: #5FB878;display:none;">&#xe610;</i>');
        tr.push('<i class="layui-icon layui-item-error" style="font-size: 20px;color:#FF5722;display:none;">&#x1007;</i>');
        tr.push('</td>');
    ...
    function initTable(otableid){
        ...
        var tfoot = [
            ...
            '<td colspan="4">',
            '<span><span class="x-red">*</span>注：添加文件后请点击上传文件按钮，上传进度条仅供参考，上传结果以状态为准</span>',
            '</td>',
            '</tr></tfoot>'
        ].join('');
        $table.append(tfoot);
    ...
    function changeNoFileBox(otableid, index){
        var nofile_tr = [
            ...
            '<div class="" style="font-size:16px;cur"><i class="layui-icon" style="font-size:26px;vertical-align: middle;">&#xe608;</i> 添加文件（直接拖拽文件到此区域）</div>',
            '</td></tr>'
        ];
