...
<div class="x-body">
    ...
    <xblock>
        ...
        <button class="layui-btn" onclick="x_admin_show('增加轮播图','<?php echo U('NewsBanner/addNewsBannerShow');?>', 1000, 800)">
...
<script>
    ...
    $('body').on('click', '#moveUp', function () {
        ...
        layer.confirm('确认要上移吗？', function () {
            $.post("<?php echo U('NewsBanner/moveUp');?>", {
                id: id
            }, function (res) {
                if (res == 200) {
                    layer.msg('已上移，请刷新查看', {icon: 1, time: 1500});
                } else if (res == 0) {
                    layer.msg('上移失败', {icon: 2, time: 1500});
                }
            });
            return false;
        });
    ...
    function member_del(obj, id) {
        layer.confirm('确认要删除吗？', function () {
            //发异步删除数据
            $.post("<?php echo U('NewsBanner/delete');?>", {
                id: id
            }, function (res) {
                if (res == 200) {
    ...
    function delAll() {
        ...
        if (data != '') {
            layer.confirm('确认要删除吗？', function () {
                //捉到所有被选中的，发异步进行删除
                $.post("<?php echo U('NewsBanner/delAll');?>", {
                    data: data
                }, function (res) {
                    if (res == 200) {
                        layer.msg('删除成功', {icon: 1, time: 1500});
